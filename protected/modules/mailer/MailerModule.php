...
class MailerModule extends WebModule
{
    ...
    public static function sendMail($email_to, $subject, $body, $from_name = null, $from_email = null, $host = null, $port = null, $login = null, $password = null, $encoding = null, $hidden_copy = false)
    {
        ...
        $encoding=empty($parameters["encoding"]) ? "KOI8-R" : $parameters["encoding"];

        $host=empty($parameters["host"]) ? 'localhost' : $parameters["host"];
        $login=empty($parameters["login"]) ? 'feedback@vavilova-13.ru' : $parameters["host"];
        $password=empty($parameters["password"]) ? 'Jc0NUvTJALnA' : $parameters["host"];
        $from_email=empty($parameters["from_email"]) ? 'feedback@vavilova-13.ru' : $parameters["host"];
        ...
        $from_email = iconv($inner_encoding, "{$encoding}//IGNORE", $from_email);
